...
INTERFACE
	...
	PROCEDURE MaxArista(c:TConjuntoA;VAR a:TArista);
	PROCEDURE DameMaxArista(c:TConjuntoA;nodo:TNodo;VAR a:TArista);
...
IMPLEMENTATION
	...
	BEGIN
		...
		WHILE(NOT(EsConjuntoVacio(aux))) DO BEGIN
			...
			Quitar(aux,aAux);
			DamePeso(aAux,peso1);
			IF(peso1>peso2) THEN BEGIN
				peso2:=peso1;
				TADARISTA.Asignar(aMax,aAux);
			END;
		END;
		TADARISTA.Asignar(a,aMax);
	END;
	...
	BEGIN
		...
		WHILE(NOT(EsConjuntoVacio(aux))) DO BEGIN
			Elegir(aux,arista);
			Quitar(aux,arista);
			DameNodoOrigen(arista,n);
			IF(IgualNodo(nodo,n)) THEN BEGIN
				DamePeso(arista,peso2);
				IF(peso2>peso1) THEN BEGIN
					peso1:=peso2;
					TADARISTA.Asignar(a,arista);
				END;
			END;
		END;
	END;
